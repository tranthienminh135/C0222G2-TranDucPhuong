...
public class EmployeeRepositoryImpl implements IEmployeeRepository {
    ...
    private final String SELECT_ALL_EMPLOYEE_IS_ACTIVE = " SELECT * FROM employee where status = 0; ";
    ...
    private final String UPDATE_EMPLOYEE = " UPDATE `employee` " +
            " SET `employee_name` = ?, `employee_birthday` = ?, `employee_id_card` = ?, `employee_salary` = ?, `employee_phone` = ?, `employee_email` = ?, `employee_address` = ?, `position_id` = ?, `education_degree_id` = ?, `division_id` = ?, `username` = ?, `status` = ? " +
            " WHERE (`employee_id` = ?); ";
    private final String SELECT_EMPLOYEE_BYNAME = " select * from employee where employee_name like ? and `status` = 0 ";
    ...
    public List<Employee> getAllEmployeeIsActive() {
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(SELECT_ALL_EMPLOYEE_IS_ACTIVE);
    ...
    public Employee getEmployeeForEdit(int idEdit) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                String username = resultSet.getString("username");
                Integer status = resultSet.getInt("status");
                employee = new Employee(employeeId, employeeName, employeeBirthday, employeeIdCard, employeeSalary, employeePhone, employeeEmail, employeeAddress, positionId, educationDegreeId, divisionId, username, status);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public List<Employee> searchAllEmployeeByName(String searchValue) {
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(SELECT_EMPLOYEE_BYNAME);
